...
class Node:
    ...
    def display_path(self) -> None:
        ...
        while temp:
            draw_char(possible_chars[-2], temp.coords[::-1], unscale=True)
            temp = temp.parent
    ...
    def display(self) -> None:
        ...
        draw_char(possible_chars[-3], self.coords[::-1], unscale=True)
...
def A_Star(distance_func: Callable = manhattan_distance, heuristic_func: Callable = lambda _: 1,
    ...
    if chase_hubs and starting_coords == config.CHOSEN_START[::-1]:
        ...
        target_hubs: list[tuple[int, int]] = [coords[::-1] for coords in config.chosen_hubs if visual_char_at(coords, True) == possible_chars[3]]
...
def A_Star_M() -> None:
    ...
    return A_Star(heuristic_func=lambda a: -1 / max(a))
