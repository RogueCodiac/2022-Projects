...
def clear() -> None:
    ...
    window.fill(possible_chars[-1], fgcolor=config.NODE_COLORS[possible_chars[-1]], bgcolor=BACKGROUND_COLOR)
...
def draw_char(char: str, coords: tuple[int, int], unscale: bool = False) -> None:
    ...
    try:
        ...
        if char == possible_chars[-1]:  # Empty space
            if char_at_coords == possible_chars[0]:  # Erase the starting node.
                config.CHOSEN_START = ()

            elif char_at_coords == possible_chars[1]:  # Erase the end node.
                config.CHOSEN_END = ()

            __draw_char(char, coords, NODE_COLORS[char])

        elif char_at_coords in possible_chars[:2]:
            return

        elif char == possible_chars[0]:  # Starting node
            input.selected_char = possible_chars[1 + bool(config.CHOSEN_END)]
            config.CHOSEN_START = __scale_coordinates(coords)

            __draw_char(char, coords, NODE_COLORS[char])

        elif char == possible_chars[1]:  # End node
            input.selected_char = possible_chars[2]
            config.CHOSEN_END = __scale_coordinates(coords)

            __draw_char(char, coords, NODE_COLORS[char])

        elif char == possible_chars[-3]:  # Search paths
            if char_at_coords == possible_chars[3]:
                __draw_char(possible_chars[3], coords, NODE_COLORS[f"{possible_chars[3]}FOUND"], no_register=True)

            elif visual_char_at(coords) != possible_chars[-2]:
                __draw_char(char, coords, NODE_COLORS[char])

        elif char == possible_chars[-2]:  # Final path
            if char_at_coords == possible_chars[3]:  # Check if node is hub.
                __draw_char(possible_chars[3], coords, NODE_COLORS[f"{possible_chars[3]}PASSED"], no_register=True)

            else:
                __draw_char(char, coords, NODE_COLORS[char])

        elif char_at_coords == possible_chars[-1] or not possible_chars[-1]:  # Check if the space is empty, draw char if so.
            __draw_char(char, coords, NODE_COLORS[char])
...
def __valid_line(line: str) -> bool:
    ...
    for c in line:
        ...
        elif c == possible_chars[0]:
            if has_start:
                return False

            has_start = True

        elif c not in possible_chars:
            return False
...
def mark_cell(coords: tuple[int, int]) -> None:
    ...
    draw_char(possible_chars[-3], coords, unscale=True)
    ...
    with open(file_name, 'r') as file:
        for line_index in range(RES[1]):
            ...
            for i, c in enumerate(line):
                ...
                if c == possible_chars[0]:
                    config.CHOSEN_START = (line_index, i)

                elif c == possible_chars[1]:
...
def regress() -> None:
    ...
    for row_index, row in enumerate(config.MAP):
        for column_index, column in enumerate(row):
            ...
            if char == possible_chars[-3] or char == possible_chars[-2]:
                draw_char(possible_chars[-1], (column_index, row_index), unscale=True)

            elif char == possible_chars[3]:
                draw_char(possible_chars[-1], (column_index, row_index), unscale=True)
                draw_char(possible_chars[3], (column_index, row_index), unscale=True)
...
def virtual_regress() -> None:
    ...
    for row_index, row in enumerate(config.MAP):
        for column_index, column in enumerate(row):
            ...
            if char == possible_chars[-3] or char == possible_chars[-2]:
                __register_display(possible_chars[-1], (column_index, row_index))
